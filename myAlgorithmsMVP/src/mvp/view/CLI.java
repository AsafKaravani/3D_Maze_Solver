...
public class CLI implements Runnable {
	...
	/**
	 * @author Yaniv and Asaf
	 * @return Gets from the user a command and from the HashMap go to its
	 *         command and use its method
	 */
			...
			if (commandName.equals("exit")) {
				...
				for (n = splitedCommand.length - 1; n >= 0; n--) {
					...
					if (commandMap.containsKey(connectedCommand.toString())) {
						...
						return commandMap.get(connectedCommand.toString()).insertArgs(args);
